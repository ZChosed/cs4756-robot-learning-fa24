...
CS 4756 Fall 2024 \hfill Due 11:59pm Thursday,  October 24}}\end{center}
\vspace{5mm}
...
\noindent{\bf (a)} 
Show the following step in \ref{eq:6} holds true:
...
\noindent and explain why this step is valid.
\newline

\noindent{\bf (b)} 
Starting from Equation \ref{eq:4}, use the causality assumption (that future actions do not affect past rewards) to derive the "reward-to-go" formulation given in Equation \ref{eq:5}. Show the intermediate steps in your derivation.
\newline

\noindent{\bf (c)} 
Introduce a baseline $b(s_t)$ to reduce the variance of the policy gradient estimator, leading to the advantage function $A(s_t, a_t) = Q(s_t, a_t) - b(s_t)$. Show that subtracting a baseline does not introduce bias in the gradient estimation, i.e., prove that:

\begin{equation}
\nabla_\theta J(\theta) = \mathbb{E}_{\tau \sim \pi_\theta(\tau)} \left[\sum_{t=0}^{T_\tau - 1} \nabla_\theta \log \pi_\theta(a_t | s_t) \cdot A(s_t, a_t)\right]
\end{equation}

\noindent Explain why the expectation of the baseline term vanishes.
\newpage
...
\noindent{\bf (d)} 
Use policy gradients to compute the gradient of the expected return of $\pi_\theta$ with respect to the parameter $\theta$ (Eq. \ref{eq:4}). Do not use discounting. The sum should telescope to a closed form solution.
\newline
You may find this fact useful:
$$\sum^{\infty}_{k=1}k\alpha^{k-1} = \frac{d}{d\alpha}\sum^{\infty}_{k=1}\alpha^{k} $$
\newline
\noindent{\bf (e)} Compute the expected return of the policy $\pi_\theta$ directly (Eq. \ref{eq:1}). Compute the gradient of this expression and verify that it matches your result in {\bf (d)}.
\newline
...
\newpage
\vskip \gap \noindent{\bf (3) Reward Shaping with an Approximate Value Function (5 points)} 
\newline
...
\newpage
\vskip \gap \noindent{\bf (4) (Mandatory for 5756): Off Policy Gradient Estimation (10 points)}
\newline
